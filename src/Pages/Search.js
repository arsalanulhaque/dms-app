...
function Search() {
  ...
  const onItemScan = (e) => {
    if (e.key === "Enter" || e.keyCode === 13) {
      makeRequest(
        `device/barcode/${inputRef.current.value}/schoolID/${getSession()?.schoolID}`,
        "get",
        undefined,
      );
    }
  ...
  const makeRequest = (api, method, body) => {
    FetchData(api, method, body, (response) => {
      if (response?.error) {
        setAlertType("danger");
        setMessage(response?.data?.message || "No Device found");
      } else if (
        ...
        // No device matched the scanned Asset ID / Barcode
        setAlertType("danger");
  ...
  const addToAvailableDevices = (device) => {
    if (device.Username === null) {
      if (
        !isDeviceInList(device, lstAvailableDevices) &&
        ...
        spreaded.sort((a, b) =>
          a.IsIssued > b.IsIssued ? 1 : b.IsIssued > a.IsIssued ? -1 : 0,
        );
  ...
  const addToIssuedDevices = (device) => {
    if (
      device.IsIssued === 1 &&
      device.FKUserID === getSession()?.data[0].UserID
    ) {
      if (
        !isDeviceInList(device, lstIssuedDevices) &&
  ...
  const isDeviceInList = (device, list) => {
    return list.findIndex((item) => item.AssetID === device.AssetID) === -1
      ? false
      : true;
  };
